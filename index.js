...
const { logRequest } = require('./middleware/logger'); // se trajo
const { verificarToken } = require('./middleware/auth'); // se trajo
...
app.post("/login", async (req, res) => {
  try {
    ...
    //const token = jwt.sign({ email }, "az_AZ", { expiresIn: "1h" }); // Token expira en 1 hora
